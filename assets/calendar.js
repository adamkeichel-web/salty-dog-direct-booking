...
function rateForDate(value){
  ...
  const baseRate=hasDiscount?Number(pricing.nightlyRate)*(1-discount):season?(weekend&&Number(season.weekendRate)>0?season.weekendRate:season.nightlyRate):(weekend&&Number(pricing.weekendRate)>0?pricing.weekendRate:pricing.nightlyRate),adjustment=Number(special?.adjustment||0),rate=baseRate*(Number.isFinite(adjustment)?1+adjustment:1);
