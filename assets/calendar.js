...
function rateForDate(value){
  ...
  const seasons=Array.isArray(pricing.seasonalRates)?pricing.seasonalRates:[],season=seasons.find(rate=>value>=rate.start&&value<rate.end),date=new Date(`${value}T00:00:00Z`),weekend=date.getUTCDay()===5||date.getUTCDay()===6,monthDay=value.slice(5),special=(Array.isArray(pricing.specialDiscounts)?pricing.specialDiscounts:[]).find(rule=>monthDay>=rule.start&&monthDay<rule.end),discountWindow=monthDay>=String(pricing.discountStart||"")&&monthDay<String(pricing.discountEnd||"99-99"),discount=Number(special?.discount??(discountWindow?(weekend?pricing.weekendDiscount:pricing.weekdayDiscount):0)),hasDiscount=Number.isFinite(discount)&&discount>0&&discount<1;
