...
mount.innerHTML=`<div class="availability-card"><div class="calendar-head"><div><div class="eyebrow" style="color:var(--sea)">Combined calendar</div><h2>Current Availability</h2><div class="calendar-status" id="calendarStatus">Please note: Availability is updated through synced calendars. Rare syncing delays may occur, so dates are not guaranteed until your booking is confirmed.</div><div class="calendar-note" id="calendarPriceStatus"></div></div><div class="calendar-nav"><button id="calendarPrev" aria-label="Previous month">‹</button><button id="calendarNext" aria-label="Next month">›</button></div></div><div class="calendar-months" id="calendarMonths"></div><div class="calendar-legend"><span><i class="legend-dot" style="background:var(--mist)"></i>Available</span><span><i class="legend-dot" style="background:#e1e7e9;border:1px solid #cfd8dc"></i>Booked or blocked</span><span><i class="legend-dot" style="background:repeating-linear-gradient(135deg,#f0f2f3,#f0f2f3 3px,#dfe4e6 3px,#dfe4e6 5px)"></i>Past date</span><span><i class="legend-dot" style="background:var(--navy)"></i>Selected</span></div></div>`;
document.querySelector(".gallery").after(mount);
const months=mount.querySelector("#calendarMonths"),status=mount.querySelector("#calendarStatus"),priceStatus=mount.querySelector("#calendarPriceStatus"),checkin=document.getElementById("checkin"),checkout=document.getElementById("checkout");
...
function rateForDate(value){
  ...
  const seasons=Array.isArray(pricing.seasonalRates)?pricing.seasonalRates:[],season=seasons.find(rate=>value>=rate.start&&value<rate.end),seasonalRate=value.slice(0,7)==="2026-09"?null:season,date=new Date(`${value}T00:00:00Z`),weekend=date.getUTCDay()===5||date.getUTCDay()===6,monthDay=value.slice(5),special=(Array.isArray(pricing.specialDiscounts)?pricing.specialDiscounts:[]).find(rule=>monthDay>=rule.start&&monthDay<rule.end),discountWindow=monthDay>=String(pricing.discountStart||"10-01")&&monthDay<String(pricing.discountEnd||"11-01"),discount=Number(special?.discount??(discountWindow?(weekend?pricing.weekendDiscount:pricing.weekdayDiscount):0)),hasDiscount=Number.isFinite(discount)&&discount>0&&discount<1;
  const baseRate=hasDiscount?Number(pricing.nightlyRate)*(1-discount):seasonalRate?(weekend&&Number(seasonalRate.weekendRate)>0?seasonalRate.weekendRate:seasonalRate.nightlyRate):(weekend&&Number(pricing.weekendRate)>0?pricing.weekendRate:pricing.nightlyRate),adjustment=Number(special?.adjustment||0),rate=baseRate*(Number.isFinite(adjustment)?1+adjustment:1);
...
fetch(`/api/direct-booking/config/${encodeURIComponent(slug)}`).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{pricing=data.showCalendarPricing?data:null;if(pricing)priceStatus.textContent="Available dates show the current direct-booking nightly rate.";render()}).catch(()=>{});
fetch(`/api/calendar/${encodeURIComponent(slug)}`).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{ranges=data.ranges||[];render()}).catch(()=>{status.textContent="Live calendar is temporarily unavailable. Please try again shortly.";priceStatus.textContent="";render()});
render();
})();
