...
fetch(`/api/calendar/${encodeURIComponent(slug)}`).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{ranges=data.ranges||[];status.textContent="";render()}).catch(()=>{status.textContent="Live calendar is temporarily unavailable—use Airbnb to check dates.";render()});
render();
})();
